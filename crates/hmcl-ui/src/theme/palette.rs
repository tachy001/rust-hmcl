...
fn with_accent(appearance: Appearance, primary: [u8; 3], on_primary: [u8; 3], container: [u8; 3], on_container: [u8; 3]) -> MonetPalette {
    let mut palette = blue(appearance);
    ...
    palette.on_primary_container = Color32::from_rgb(on_container[0], on_container[1], on_container[2]);
    palette
}

fn darker_blue(appearance: Appearance) -> MonetPalette {
    if appearance.is_dark() {
        with_accent(appearance, [0x9F, 0xA8, 0xFF], [0x00, 0x06, 0x60], [0x20, 0x2E, 0x7A], [0xDE, 0xE0, 0xFF])
    } else {
        with_accent(appearance, [0x28, 0x35, 0x93], [0xFF, 0xFF, 0xFF], [0x1A, 0x27, 0x8E], [0xDE, 0xE0, 0xFF])
    }
}

fn green(appearance: Appearance) -> MonetPalette {
    if appearance.is_dark() {
        with_accent(appearance, [0x89, 0xD9, 0x80], [0x00, 0x39, 0x0B], [0x14, 0x5F, 0x26], [0xB2, 0xF2, 0xB2])
    } else {
        with_accent(appearance, [0x43, 0xA0, 0x47], [0xFF, 0xFF, 0xFF], [0x30, 0x87, 0x36], [0xE2, 0xF2, 0xDF])
    }
}

fn orange(appearance: Appearance) -> MonetPalette {
    if appearance.is_dark() {
        with_accent(appearance, [0xFF, 0xB7, 0x75], [0x4D, 0x2A, 0x00], [0x7A, 0x48, 0x00], [0xFF, 0xDC, 0xC2])
    } else {
        with_accent(appearance, [0xE6, 0x7E, 0x22], [0xFF, 0xFF, 0xFF], [0xC5, 0x63, 0x00], [0xFF, 0xE1, 0xC7])
    }
}

fn purple(appearance: Appearance) -> MonetPalette {
    if appearance.is_dark() {
        with_accent(appearance, [0xE8, 0xB9, 0xFF], [0x49, 0x07, 0x5F], [0x62, 0x20, 0x78], [0xF5, 0xD9, 0xFF])
    } else {
        with_accent(appearance, [0x9C, 0x27, 0xB0], [0xFF, 0xFF, 0xFF], [0x80, 0x00, 0x96], [0xF5, 0xD9, 0xFF])
    }
}

fn red(appearance: Appearance) -> MonetPalette {
    if appearance.is_dark() {
        with_accent(appearance, [0xFF, 0xB3, 0xB3], [0x68, 0x00, 0x08], [0x8F, 0x0E, 0x1C], [0xFF, 0xDA, 0xD9])
    } else {
        with_accent(appearance, [0xB7, 0x1C, 0x1C], [0xFF, 0xFF, 0xFF], [0x98, 0x00, 0x0E], [0xFF, 0xDA, 0xD9])
    }
}
